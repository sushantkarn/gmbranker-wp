...
(function($) {
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.gmb-sidebar-nav-item', function(e) {
            ...
            $container.find('.gmb-subtab-panel').removeClass('active is-active').removeAttr('style').hide();
            
            // Show selected panel
            var $target = $('#' + targetSub);
            if ($target.length) {
                $target.addClass('active').attr('style', 'display: block !important;').show();
            }

            // Sync URL parameter
            if (window.history && window.history.replaceState) {
                var cleanSub = targetSub.replace('gmb-subtab-sitemap-', '').replace('gmb-subtab-schema-', '').replace('gmb-subtab-', '');
        ...
        $(document).on('change', '.gmb-type-selector input[type="radio"]', function() {
    ...
    window.gmbSyncAnalytics = function() {
        ...
        var ajaxUrl = (typeof gmb_ranker_admin !== 'undefined' && gmb_ranker_admin.ajax_url) ? gmb_ranker_admin.ajax_url : '/wp-admin/admin-ajax.php';
        var nonce = (typeof gmb_ranker_admin !== 'undefined' && gmb_ranker_admin.nonce) ? gmb_ranker_admin.nonce : '';
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success && response.data && response.data.data) {
                    ...
                    if (d.totals) {
                        if (d.totals.clicks) $('#gmb-kpi-clicks').text(Number(d.totals.clicks).toLocaleString());
                        if (d.totals.impressions) $('#gmb-kpi-impressions').text(Number(d.totals.impressions).toLocaleString());
                        if (d.totals.ctr) $('#gmb-kpi-ctr').text(d.totals.ctr + '%');
                        if (d.totals.position) $('#gmb-kpi-pos').text(d.totals.position);
                    }
                    if (d.status === 'connected') {
                        $('#gmb-analytics-status-badge').removeClass('gmb-analytics-badge-preview').addClass('gmb-analytics-badge-connected').html('<svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="12" r="10"/></svg> Live Cloud Sync');
                    }
                    $label.text('Synced!');
                    setTimeout(function() {
                        $label.text('Sync Live Data');
