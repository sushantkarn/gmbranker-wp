            ...
            window.fetch = function(input, init) {
                if (typeof input === 'string' && input.includes('admin-ajax.php') && init && init.body) {
                    let bodyStr = '';
                    if (init.body instanceof URLSearchParams) {
                        ...
                        bodyStr = init.body;
                        if (bodyStr.includes('action=gmb_') && !bodyStr.includes('nonce=')) {
                            init.body += '&nonce=' + encodeURIComponent(window.gmb_ranker_admin.nonce);
                        }
                    }
                }
                ...
                jQuery(document).ajaxSend(function(event, xhr, settings) {
                    if (settings.data) {
                        if (typeof settings.data === 'string') {
                            if (settings.data.indexOf('action=gmb_') !== -1 && settings.data.indexOf('nonce=') === -1) {
                                settings.data += '&nonce=' + encodeURIComponent(window.gmb_ranker_admin.nonce);
                            }
                        } else if (typeof settings.data === 'object') {
                            if (settings.data.action && settings.data.action.indexOf('gmb_') === 0 && !settings.data.nonce) {
            ...
            function initGmbAdminApp() {
                ...
                function initGmbSidebarNavigation() {
                    ...
                    sidebarItems.forEach(item => {
                        item.addEventListener('click', function(e) {
                            ...
                            const container = item.closest('.gmb-sidebar-layout-container') || document;
                            ...
                            const cleanSub = targetPanelId.replace('gmb-subtab-sitemap-', '').replace('gmb-subtab-schema-', '').replace('gmb-subtab-', '');
                            ...
                            const pageParam = currentUrl.searchParams.get('page') || 'gmb-ranker-settings';
                            try {
                                sessionStorage.setItem('gmb_active_subtab_' + pageParam, cleanSub);
                            } catch (e) {}

                            // Update active subtab hidden inputs in forms
                            const subtabInput = container.querySelector('input[name="gmb_active_subtab"]') || document.querySelector('input[name="gmb_active_subtab"]');
                            if (subtabInput) {
                                subtabInput.value = cleanSub;
                            }

                            // Update _wp_http_referer to preserve active subtab on redirect after options.php save
                            const refererInputs = document.querySelectorAll('input[name="_wp_http_referer"]');
                ...
                if (pageParam === 'gmb-ranker-settings') {
                    ...
                    const subtabBtn = document.querySelector(`.gmb-sidebar-nav-item[data-subtab="${subtabSelector}"]`);
                    if (subtabBtn) {
                        subtabBtn.click();
                    }
                ...
                const schemaLogoPreviewWrap = document.getElementById('gmb_schema_logo_preview_wrap');
                ...
                const schemaDefImgBtn = document.getElementById('gmb_schema_upload_default_img_btn');
                const schemaDefImgInput = document.getElementById('gmb_schema_default_img_input');
                const schemaDefImgPreview = document.getElementById('gmb_schema_default_img_preview');
                const schemaDefImgPreviewWrap = document.getElementById('gmb_schema_default_img_preview_wrap');
                ...
                if (testGoogleBtn && testUrlInput) {
                    testGoogleBtn.addEventListener('click', function() {
                        const targetUrl = encodeURIComponent(testUrlInput.value.trim() || window.location.origin);
                        window.open(`https://search.google.com/test/rich-results?url=${targetUrl}`, '_blank');
                    });
                }
                ...
                // Modal Triggers
                const schemaTplOpenBtn = document.getElementById('gmb-open-new-template-modal-btn');
                const schemaTplEmptyBtn = document.getElementById('gmb-empty-create-template-btn');
                const schemaTplCancelBtn = document.getElementById('gmb-cancel-template-modal-btn');
                const schemaTplSaveBtn = document.getElementById('gmb-save-template-modal-btn');
                ...
                const tplActiveBadge = document.getElementById('gmb-builder-active-type-badge');
                ...
                const tplSyntaxIndicator = document.getElementById('gmb-json-syntax-indicator');
                const tplConditionsContainer = document.getElementById('gmb-modal-conditions-container');
                ...
                // Catalog Controls
                const catalogSearchInput = document.getElementById('gmb-catalog-search-input');
                const catalogCardsGrid = document.getElementById('gmb-catalog-cards-grid');
                const savedTemplatesGrid = document.getElementById('gmb-saved-templates-grid');
                const catalogNoResults = document.getElementById('gmb-catalog-no-results');
                const catalogRadioCatalog = document.getElementById('gmb-catalog-radio-catalog');
                ...
                const builderTabConditions = document.getElementById('gmb-builder-tab-btn-conditions');
                ...
                const builderPanelConditions = document.getElementById('gmb-builder-panel-conditions');
                ...
                const btnAddPropertyGroup = document.getElementById('gmb-btn-add-property-group');
                ...
                const gmb_schema_blueprints = {
                    ...
                    'Event': JSON.stringify({
                        ...
                        "description": "%excerpt%",
                        "startDate": "%date%",
                        "location": { "@type": "Place", "name": "%sitename%", "address": "%street%, %locality%" }
                    }, null, 2),
                    'FAQPage': JSON.stringify({
                        ...
                        "mainEntity": [
                            ...
                            },
                            {
                                ...
                                "acceptedAnswer": { "@type": "Answer", "text": "Contact us at %phone% or visit our website at %siteurl%." }
                            }
                        ]
                    }, null, 2),
                    ...
                    'HowTo': JSON.stringify({
                        ...
                        "description": "%excerpt%",
                        "step": [
                            { "@type": "HowToStep", "name": "Initial Assessment", "text": "Schedule an initial evaluation and consultation." },
                            { "@type": "HowToStep", "name": "Treatment or Delivery", "text": "Receive specialized care or services tailored to your needs." }
                        ]
                    }, null, 2),
                ...
                function switchCatalogTab(tabKey) {
                    ...
                    if (tabKey === 'custom') {
                        const previewArea = document.getElementById('gmb-custom-preview-textarea');
                        const presetSelect = document.getElementById('gmb-custom-preset-select');
                        if (previewArea && (!previewArea.value.trim() || previewArea.value === '')) {
                            const pKey = presetSelect ? presetSelect.value : 'blank';
                            previewArea.value = gmb_custom_schema_presets[pKey] || gmb_custom_schema_presets['blank'];
                        }
                    }
                }

                if (catalogTabTemplates) catalogTabTemplates.addEventListener('click', () => switchCatalogTab('templates'));
                if (catalogTabImport) catalogTabImport.addEventListener('click', () => switchCatalogTab('import'));
                if (catalogTabCustom) catalogTabCustom.addEventListener('click', () => switchCatalogTab('custom'));

                // Import Panel Controls
                const importSourceSelect = document.getElementById('gmb-catalog-import-source');
                const importCodeLabel = document.getElementById('gmb-catalog-import-code-label');
                const importCodeWrap = document.getElementById('gmb-catalog-import-code-wrap');
                ...
                const importUrlInput = document.getElementById('gmb-catalog-import-url-input');
                const importError = document.getElementById('gmb-catalog-import-error');
                const importProcessBtn = document.getElementById('gmb-catalog-import-process-btn');
                ...
                if (importSourceSelect) {
                    importSourceSelect.addEventListener('change', function() {
                        ...
                        if (this.value === 'jsonld') {
                            ...
                            if (importCodeLabel) importCodeLabel.textContent = 'Custom JSON-LD Code';
                            if (importTextarea) importTextarea.placeholder = '{"@context": "https://schema.org", "@type": "MedicalClinic", ...}';
                            if (importUrlWrap) importUrlWrap.style.display = 'none';
                        } else if (this.value === 'html') {
                            if (importCodeWrap) importCodeWrap.style.display = 'block';
                            if (importCodeLabel) importCodeLabel.textContent = 'HTML / Webpage Source Code (containing JSON-LD)';
                            if (importTextarea) importTextarea.placeholder = '<html><head><script type="application/ld+json">{"@context":"https://schema.org", "@type":"FAQPage", ...}<' + '/script></head>...';
                            if (importUrlWrap) importUrlWrap.style.display = 'none';
                ...
                if (importProcessBtn) {
                    importProcessBtn.addEventListener('click', function() {
                        ...
                        if (source === 'url') {
                            ...
                            if (!urlVal) {
                                if (importError) {
                                    importError.textContent = 'Please enter a valid webpage URL to extract Schema.';
                                    importError.style.display = 'block';
                                }
                                return;
                            }
                            if (importError) {
                                importError.textContent = 'To import schema directly, please view page source in your browser and paste the JSON-LD or HTML code into the editor above.';
                                importError.style.display = 'block';
                            }
                            return;
                        }
                        ...
                        if (!rawCode) {
                            if (importError) {
                                importError.textContent = 'Please paste your Schema JSON-LD or HTML source code.';
                                importError.style.display = 'block';
                            }
                            return;
                        }
                        ...
                        if (source === 'html' || rawCode.includes('<' + 'script')) {
                            const scriptMatch = rawCode.match(new RegExp('<' + 'script[^>]*type=["\']application\\/ld\\+json["\'][^>]*>([\\s\\S]*?)<' + '\\/script>', 'i'));
                            if (scriptMatch && scriptMatch[1]) {
                                jsonStr = scriptMatch[1].trim();
                            } else {
                                jsonStr = rawCode.replace(new RegExp('<' + 'script[^>]*>', 'gi'), '').replace(new RegExp('<' + '\\/script>', 'gi'), '').trim();
                            }
                        }
                        ...
                        try {
                            ...
                            const detectedName = schemaObj.name || schemaObj.headline || (detectedType + ' Schema');
                            ...
                            if (importError) {
                                importError.textContent = 'Could not parse JSON-LD: ' + err.message + '. Please ensure valid JSON format.';
                                importError.style.display = 'block';
                            }
                        }
                    });
                ...
                const customPresetSelect = document.getElementById('gmb-custom-preset-select');
                const customPreviewTextarea = document.getElementById('gmb-custom-preview-textarea');
                ...
                if (customPresetSelect && customPreviewTextarea) {
                    customPresetSelect.addEventListener('change', function() {
                        ...
                        const starter = gmb_custom_schema_presets[presetKey] || gmb_custom_schema_presets['blank'];
                        ...
                        try {
                            ...
                            if (customTitleInput) {
                                customTitleInput.value = (parsed['@type'] || 'Custom') + ' Schema Blueprint';
                            }
                        } catch(e) {}
                    });
                }
                ...
                if (customCreateBtn) {
                    customCreateBtn.addEventListener('click', function() {
                        const customType = (customTypeInput ? customTypeInput.value.trim() : '') || 'Custom';
                        const customTitle = (customTitleInput ? customTitleInput.value.trim() : '') || (customType + ' Schema Blueprint');
                        let customJson = customPreviewTextarea ? customPreviewTextarea.value.trim() : '';
                ...
                function switchBuilderTab(tabKey) {
                    const tabs = [
                        ...
                        { key: 'conditions', btn: builderTabConditions, panel: builderPanelConditions }
                    ];
                ...
                if (builderTabEdit) builderTabEdit.addEventListener('click', () => switchBuilderTab('edit'));
                if (builderTabCode) builderTabCode.addEventListener('click', () => switchBuilderTab('code'));
                if (builderTabConditions) builderTabConditions.addEventListener('click', () => switchBuilderTab('conditions'));
                ...
                if (schemaTplCancelBtn) schemaTplCancelBtn.addEventListener('click', closeSchemaModal);
                ...
                function openCatalogModal() {
                    ...
                    document.querySelectorAll('.gmb-schema-template-card, .gmb-schema-card').forEach(c => c.style.display = 'flex');
                ...
                if (schemaTplOpenBtn) schemaTplOpenBtn.addEventListener('click', openCatalogModal);
                if (schemaTplEmptyBtn) schemaTplEmptyBtn.addEventListener('click', openCatalogModal);
                ...
                function openBuilderModal(type, isEdit, data, customTitle, customJson) {
                    ...
                    if (customJson) {
                        document.getElementById('gmb-builder-modal-title').textContent = 'Schema Builder';
                        tplIdInput.value = '';
                        ...
                        document.getElementById('gmb-builder-modal-title').textContent = 'Edit Schema: ' + (data.title || 'Untitled');
                        ...
                        if (tplStatusLabel) {
                            tplStatusLabel.textContent = isActive ? 'Active Template' : 'Inactive Template';
                        ...
                        tplJsonArea.value = gmb_schema_blueprints[type] || gmb_schema_blueprints['Custom'];
                        renderConditionRow({ type: 'include', target: 'entire_site', value: '' });
                    }
                ...
                document.querySelectorAll('.gmb-schema-template-card, .gmb-schema-card').forEach(card => {
                ...
                if (catalogSearchInput) {
                    catalogSearchInput.addEventListener('input', function() {
                        ...
                        const cards = document.querySelectorAll('.gmb-schema-template-card, .gmb-schema-card');
                        let matchCount = 0;
                        cards.forEach(c => {
                ...
                function renderSavedTemplatesInCatalog() {
                    ...
                    const rows = document.querySelectorAll('#gmb-schema-templates-table tbody tr[id^="gmb-tpl-row-"]');
                    if (!rows.length) {
                        savedTemplatesGrid.innerHTML = '<div class="gmb-cat-empty-state gmb-col-span-2">No saved templates yet. Click on any card in the Schema Catalog to create one.</div>';
                        return;
                    }
                    rows.forEach(r => {
                ...
                if (tplCopyJsonBtn && tplJsonArea) {
                    tplCopyJsonBtn.addEventListener('click', function() {
                        ...
                        navigator.clipboard.writeText(tplJsonArea.value).then(() => {
                            const original = tplCopyJsonBtn.textContent;
                            tplCopyJsonBtn.textContent = '✓ Copied!';
                            setTimeout(() => { tplCopyJsonBtn.textContent = original; }, 1800);
                        }).catch(() => {
                            tplJsonArea.select();
                ...
                function validateSchemaTplJson() {
                    ...
                    if (!val) {
                        ...
                        tplSyntaxIndicator.className = 'gmb-ide-badge gmb-syntax-indicator--empty';
                        return false;
                    }
                    try {
                        JSON.parse(val);
                        tplSyntaxIndicator.textContent = '✓ Valid JSON-LD Syntax';
                        tplSyntaxIndicator.className = 'gmb-ide-badge gmb-syntax-indicator--valid';
                        return true;
                    } catch (err) {
                        tplSyntaxIndicator.textContent = '✕ Syntax Error: ' + err.message;
                        tplSyntaxIndicator.className = 'gmb-ide-badge gmb-syntax-indicator--invalid';
                        return false;
                    }
                }
                ...
                if (tplLoadPresetBtn && tplJsonArea && tplTypeSelect) {
                    tplLoadPresetBtn.addEventListener('click', function() {
                        ...
                        if (gmb_schema_blueprints[type]) {
                            if (tplJsonArea.value.trim() && !confirm('Reset current structured data to default ' + type + ' blueprint?')) {
                                return;
                            }
                ...
                function syncJsonToVisualBuilder() {
                    ...
                    if (type === 'FAQPage') {
                        let mainEntity = (parsedJson && Array.isArray(parsedJson.mainEntity)) ? parsedJson.mainEntity : [];
                        ...
                        function renderFaqList() {
                            ...
                            if (!mainEntity.length) {
                                visualContent.innerHTML = '<div class="gmb-faq-empty-state">No FAQ items yet. Click "+ Add Property" below to add a question.</div>';
                                return;
                            }

                            mainEntity.forEach((faq, index) => {
                                const qName = faq.name || '';
                                const aText = (faq.acceptedAnswer && faq.acceptedAnswer.text) ? faq.acceptedAnswer.text : '';
                                ...
                                card.querySelector('.gmb-del-faq-btn').addEventListener('click', function() {
                        ...
                        const sDesc = (parsedJson && parsedJson.description) ? parsedJson.description : '%excerpt%';
                        const sProvider = (parsedJson && parsedJson.provider && parsedJson.provider.name) ? parsedJson.provider.name : '%sitename%';
                        const sPhone = (parsedJson && parsedJson.provider && parsedJson.provider.telephone) ? parsedJson.provider.telephone : '%phone%';
                        const sArea = (parsedJson && parsedJson.areaServed) ? parsedJson.areaServed : '%locality%';
                        ...
                        visualContent.querySelectorAll('input, textarea').forEach(el => el.addEventListener('input', updateServiceJson));

                    } else if (type === 'LocalBusiness' || type === 'Restaurant') {
                        const bName = (parsedJson && parsedJson.name) ? parsedJson.name : '%sitename%';
                        const bPhone = (parsedJson && parsedJson.telephone) ? parsedJson.telephone : '%phone%';
                        const bEmail = (parsedJson && parsedJson.email) ? parsedJson.email : '%email%';
                        const bStreet = (parsedJson && parsedJson.address && parsedJson.address.streetAddress) ? parsedJson.address.streetAddress : '%street%';
                        const bLocality = (parsedJson && parsedJson.address && parsedJson.address.addressLocality) ? parsedJson.address.addressLocality : '%locality%';
                        ...
                        function updateBizJson() {
                            const newObj = {
                                ...
                                "address": {
                                    ...
                                    "addressLocality": document.getElementById('gmb-vis-biz-locality').value
                                }
                            };
                            tplJsonArea.value = JSON.stringify(newObj, null, 2);
                            validateSchemaTplJson();
                        ...
                        visualContent.querySelectorAll('input').forEach(el => el.addEventListener('input', updateBizJson));

                    } else {
                        // General Property List for Article, Dataset, Product, Course, Event, Custom, etc.
                        if (parsedJson && typeof parsedJson === 'object') {
                            const entries = Object.entries(parsedJson).filter(([k]) => k !== '@context' && k !== '@type');
                            if (entries.length) {
                                entries.forEach(([k, v]) => {
                                ...
                                visualContent.innerHTML = '<div class="gmb-faq-empty-state">No properties added yet. Click "+ Add Property" or "+ Add Property Group" below.</div>';
                            }
                        }
                    }
                }
                ...
                function createPropRowElement(key = '', val = '') {
                    ...
                    row.querySelectorAll('input').forEach(inp => inp.addEventListener('input', syncCustomPropsToJson));
                    return row;
                }
                ...
                function addVisualPropertyGroup(groupKey = '', groupObj = {}) {
                    ...
                    const groupType = (groupObj && groupObj['@type']) ? groupObj['@type'] : 'Thing';
                    const card = document.createElement('div');
                    ...
                    if (groupObj && typeof groupObj === 'object') {
                        const childEntries = Object.entries(groupObj).filter(([k]) => k !== '@type' && k !== '@context');
                        if (childEntries.length) {
                            childEntries.forEach(([ck, cv]) => {
                                const row = createPropRowElement(ck, (typeof cv === 'object' && cv !== null) ? JSON.stringify(cv) : cv);
                ...
                function syncCustomPropsToJson() {
                    ...
                    const directRows = visualContent.querySelectorAll(':scope > .gmb-custom-prop-row');
                    ...
                    const groupCards = visualContent.querySelectorAll(':scope > .gmb-prop-group-card');
                    groupCards.forEach(card => {
                        ...
                        if (gKey) {
                            ...
                            card.querySelectorAll('.gmb-prop-group-fields .gmb-custom-prop-row').forEach(cr => {
                ...
                if (btnAddProperty) {
                    btnAddProperty.addEventListener('click', function() {
                        ...
                        if (type === 'FAQPage') {
                            let parsed = {};
                            try { parsed = JSON.parse(tplJsonArea.value.trim()); } catch(e) {}
                ...
                function renderConditionRow(cond) {
                    ...
                    typeSelect.innerHTML = '<option value="include"' + (cond.type === 'include' ? ' selected' : '') + '>Include</option><option value="exclude"' + (cond.type === 'exclude' ? ' selected' : '') + '>Exclude</option>';

                    function updateTypeSelectStyle() {
                        typeSelect.className = 'gmb-cond-type gmb-select gmb-cond-type--' + typeSelect.value;
                    }
                    ...
                    function updateValField() {
                        ...
                        if (tgt === 'post_type') {
                            ...
                            valContainer.innerHTML = '<span class="gmb-cond-no-param">No additional parameters needed</span>';
                        }
                    }
                ...
                if (schemaTplSaveBtn) {
                    schemaTplSaveBtn.addEventListener('click', function() {
                        ...
                        const conditions = [];
                        const rows = tplConditionsContainer.querySelectorAll('.gmb-condition-rule-row');
                        rows.forEach(r => {
                            const cType = r.querySelector('.gmb-cond-type') ? r.querySelector('.gmb-cond-type').value : 'include';
                            const cTarget = r.querySelector('.gmb-cond-target') ? r.querySelector('.gmb-cond-target').value : 'entire_site';
                        ...
                        const getAdminNonce = () => {
                            return (window.gmb_ranker_admin && (window.gmb_ranker_admin.schema_nonce || window.gmb_ranker_admin.admin_nonce || window.gmb_ranker_admin.nonce)) || '';
                        };
                        const ajaxEndpoint = (typeof ajaxurl !== 'undefined') ? ajaxurl : (window.gmb_ranker_admin && window.gmb_ranker_admin.ajax_url ? window.gmb_ranker_admin.ajax_url : '/wp-admin/admin-ajax.php');
                        ...
                        .then(res => {
                            if (res.success) {
                                alert(res.data && res.data.message ? res.data.message : 'Template saved successfully!');
                                window.location.href = window.location.pathname + '?page=gmb-ranker-schema&tab=templates';
                            } else {
                                alert(res.data && res.data.message ? res.data.message : 'Error saving template.');
                    ...
                    btn.addEventListener('click', function() {
                        ...
                        .then(res => {
                            ...
                            if (res.success && res.data && res.data.template) {
                                openBuilderModal(res.data.template.type || 'Custom', true, res.data.template);
                            } else {
                    ...
                    btn.addEventListener('click', function() {
                        ...
                        .then(res => {
                            if (res.success && res.data && res.data.template) {
                                ...
                                return fetch(ajaxEndpoint, { method: 'POST', body: dupFd }).then(r2 => r2.json());
                            }
                ...
                document.querySelectorAll('.gmb-delete-template-btn').forEach(btn => {
                    btn.addEventListener('click', function() {
                        const id = this.getAttribute('data-id');
                        if (!id) return;
                        if (!confirm('Are you sure you want to permanently delete this Schema Template?')) {
                            return;
                        }
                        ...
                        .then(res => {
                            if (res.success) {
                                ...
                                alert(res.data && res.data.message ? res.data.message : 'Error deleting template.');
                                btn.disabled = false;
                            }
                        })
                        .catch(() => {
                ...
                presetBtns.forEach(btn => {
                    btn.addEventListener('click', function(e) {
                        ...
                        if (!confirm('Apply this Schema blueprint? This will update your default schema mapping and knowledge graph settings.')) {
                            return;
                        }
                        ...
                        .then(res => {
                            if (res.success) {
                                alert(res.data && res.data.message ? res.data.message : 'Preset applied successfully!');
                                window.location.reload();
                            } else {
                                alert(res.data && res.data.message ? res.data.message : 'Failed to apply preset.');
                                btn.disabled = false;
                ...
                const gmb_toggle_module_nonce = (window.gmb_ranker_admin && window.gmb_ranker_admin.toggle_module_nonce) || (window.gmb_ranker_admin && window.gmb_ranker_admin.nonce) || '';
                ...
                function saveDashboardModuleToggle(checkbox) {
                    ...
                    formData.append('action', 'gmb_toggle_dashboard_module');
                        if (typeof gmb_ranker_admin !== 'undefined') { formData.append('nonce', gmb_ranker_admin.admin_nonce || gmb_ranker_admin.nonce); }
                ...
                const toggles = document.querySelectorAll('#gmb-modules-form input[type="checkbox"]');
                toggles.forEach(t => {
                    ...
                    btn.addEventListener('click', function(e) {
                        ...
                        .then(res => {
                            if (res.success) {
                                ...
                                alert(res.data && res.data.message ? res.data.message : 'Error enabling module.');
                                btn.disabled = false;
                ...
                if (resetLlmsBtn) {
                    resetLlmsBtn.addEventListener('click', function() {
                        if (confirm('Are you sure you want to reset all LLMs Txt settings?')) {
                            ...
                            document.querySelectorAll('.llms-tax-checkbox').forEach(cb => cb.checked = false);
                            document.querySelector('input[name="gmb_llms_limit"]').value = '100';
                            document.querySelector('input[name="gmb_llms_title"]').value = (window.gmb_ranker_admin && window.gmb_ranker_admin.site_name) || '';
                            document.querySelector('textarea[name="gmb_llms_desc"]').value = (window.gmb_ranker_admin && window.gmb_ranker_admin.site_desc) || '';
                            document.querySelector('textarea[name="gmb_llms_additional_content"]').value = '';
                        }
                    });
                }

                var prefOverlay = document.getElementById('preferred-source-settings-overlay');
                ...
                if (resetAiBtn) {
                    resetAiBtn.addEventListener('click', function() {
                        if (confirm('Are you sure you want to reset Content AI settings?')) {
                            ...
                            document.querySelector('input[name="gmb_ai_openrouter_model"]').value = 'meta-llama/llama-3-8b-instruct:free';
                            document.querySelector('input[name="gmb_ai_groq_key"]').value = '';
                            document.querySelector('input[name="gmb_ai_groq_model"]').value = 'llama3-8b-8192';
                            document.querySelector('input[name="gmb_ai_ollama_url"]').value = 'http://localhost:11434';
                            document.querySelector('input[name="gmb_ai_ollama_model"]').value = 'llama3';
                        }
                    });
                }
                ...
                if (tocOpenBtn) {
                    tocOpenBtn.addEventListener('click', function() {
                        const tabBtn = document.querySelector('.rm-nav-tab[data-tab="rm-tab-toc"]');
                        if (tabBtn) {
                            tabBtn.click();
                        } else {
                ...
                const idxOverlay = document.getElementById('instant-indexing-settings-overlay');
                ...
                const toggleMultiCheck = document.getElementById('gmb-toggle-multi-locations');
                ...
                if (addLocBtn) {
                    addLocBtn.addEventListener('click', function() {
                        ...
                        const address = document.getElementById('gmb-new-loc-address').value.trim();
                ...
                if (orphanBtn) {
                    orphanBtn.addEventListener('click', function() {
                        ...
                        fetch(ajaxurl, {
                            ...
                            if (data.success) {
                                alert('Deleted ' + data.data + ' orphan postmeta entries successfully!');
                            }
                        });
                    });
                }
                ...
                function triggerRankMathImport(btn) {
                    if (!confirm('Are you sure you want to import all SEO metadata (Titles, Descriptions, Keywords, Canonicals, Robots) from Rank Math? Existing GMB Ranker metadata for posts that have Rank Math settings will be overwritten.')) {
                        return;
                    }
                    ...
                    fetch(ajaxurl, {
                        ...
                        btn.innerText = (btn.id === 'gmb-db-import-rankmath-btn') ? 'Run Tool' : 'Import Now';
                        if (data.success) {
                ...
                if (dbImportRmBtn) {
                    dbImportRmBtn.addEventListener('click', function() { triggerRankMathImport(dbImportRmBtn); });
                }
                if (mainImportRmBtn) {
                    mainImportRmBtn.addEventListener('click', function() { triggerRankMathImport(mainImportRmBtn); });
                }
                ...
                if (importYoastBtn) {
                    importYoastBtn.addEventListener('click', function() {
                        if (!confirm('Are you sure you want to import all SEO metadata (Titles, Descriptions, Keywords, Canonicals, Robots) from Yoast SEO? Existing GMB Ranker metadata for posts that have Yoast settings will be overwritten.')) {
                            return;
                        }
                ...
                if (restoreBtn) {
                    restoreBtn.addEventListener('click', function() {
                        ...
                        if (!confirm('Are you sure you want to restore settings from this backup? Current configurations will be overwritten.')) {
                            return;
                        }
                ...
                if (submitIndexingBtn) {
                    submitIndexingBtn.addEventListener('click', function() {
                        ...
                        const getAdminNonce = () => {
                            return (window.gmb_ranker_admin && (window.gmb_ranker_admin.instant_index_nonce || window.gmb_ranker_admin.admin_nonce || window.gmb_ranker_admin.nonce)) || '';
                        };
                        const ajaxEndpoint = (typeof ajaxurl !== 'undefined') ? ajaxurl : (window.gmb_ranker_admin && window.gmb_ranker_admin.ajax_url ? window.gmb_ranker_admin.ajax_url : '/wp-admin/admin-ajax.php');
                ...
                if (addRuleBtn) {
                    addRuleBtn.addEventListener('click', function() {
                        const editId = document.getElementById('gmb-redirect-edit-id') ? document.getElementById('gmb-redirect-edit-id').value.trim() : '';
                        const source = document.getElementById('gmb-redirect-source').value.trim();
                        const destination = document.getElementById('gmb-redirect-destination').value.trim();
                        ...
                        .then(data => {
                            ...
                            if (data.success) {
                                ...
                                alert('Failed to save redirection rule: ' + (data.data || 'Unknown error'));
                            }
                        })
                        .catch(err => {
                            addRuleBtn.disabled = false;
                ...
                editRuleBtns.forEach(btn => {
                    btn.addEventListener('click', function() {
                        const formContainer = document.getElementById('gmb-redirect-form-container');
                        if (formContainer) {
                            formContainer.style.display = 'block';
                            document.getElementById('gmb-redirect-form-title').innerText = 'Edit Redirection';
                            const badge = document.getElementById('gmb-redirect-edit-badge');
                            if (badge) badge.style.display = 'inline-block';
                            
                            document.getElementById('gmb-redirect-edit-id').value = btn.getAttribute('data-id') || '';
                            document.getElementById('gmb-redirect-source').value = btn.getAttribute('data-source') || '';
                            document.getElementById('gmb-redirect-destination').value = btn.getAttribute('data-dest') || '';
                            document.getElementById('gmb-redirect-code').value = btn.getAttribute('data-code') || '301';
                            document.getElementById('gmb-redirect-match-type').value = btn.getAttribute('data-match') || 'exact';
                            document.getElementById('gmb-redirect-status').value = btn.getAttribute('data-status') || 'active';
                            if (document.getElementById('gmb-redirect-note')) {
                                document.getElementById('gmb-redirect-note').value = btn.getAttribute('data-note') || '';
                            }
                ...
                createBtns.forEach(btn => {
                    btn.addEventListener('click', function() {
                        const url = btn.getAttribute('data-url');
                        const manageNavBtn = document.querySelector('.gmb-redirect-subnav[data-sub="gmb-redirect-manage"]');
                        if (manageNavBtn) {
                            manageNavBtn.click();
                        }
                        const formContainer = document.getElementById('gmb-redirect-form-container');
                        if (formContainer) {
                            formContainer.style.display = 'block';
                            document.getElementById('gmb-redirect-form-title').innerText = 'Add Redirection from 404 Log';
                ...
                const redirectFormContainer = document.getElementById('gmb-redirect-form-container');
                ...
                if (toggleAddFormBtn && redirectFormContainer) {
                    toggleAddFormBtn.addEventListener('click', function() {
                        if (redirectFormContainer.style.display === 'none') {
                            ...
                            document.getElementById('gmb-redirect-form-title').innerText = 'Add New Redirection';
                            document.getElementById('gmb-redirect-edit-id').value = '';
                            document.getElementById('gmb-redirect-source').value = '';
                            document.getElementById('gmb-redirect-destination').value = '';
                            if (document.getElementById('gmb-redirect-note')) document.getElementById('gmb-redirect-note').value = '';
                            const badge = document.getElementById('gmb-redirect-edit-badge');
                ...
                const manageTopActions = document.getElementById('gmb-redirect-manage-top-actions');
                ...
                redirectSubnavs.forEach(btn => {
                    btn.addEventListener('click', function() {
                        ...
                        if (manageTopActions) {
                            manageTopActions.style.display = (targetSub === 'gmb-redirect-manage') ? 'block' : 'none';
                        }
                    });
                });
                ...
                const redirectFilterCode = document.getElementById('gmb-filter-redirect-code');
                const redirectFilterStatus = document.getElementById('gmb-filter-redirect-status');

                function filterRedirectRules() {
                    const query = redirectSearchInput ? redirectSearchInput.value.toLowerCase().trim() : '';
                    const codeFilter = redirectFilterCode ? redirectFilterCode.value : 'all';
                    const statusFilter = redirectFilterStatus ? redirectFilterStatus.value : 'all';

                    const rows = document.querySelectorAll('.gmb-rule-row');
                    rows.forEach(row => {
                        const src = (row.querySelector('.gmb-rule-source-text')?.innerText || '').toLowerCase();
                        const dest = (row.querySelector('.gmb-rule-dest-text')?.innerText || '').toLowerCase();
                        const note = (row.querySelector('.gmb-rule-note-text')?.innerText || '').toLowerCase();
                        const code = row.getAttribute('data-code');
                        const status = row.getAttribute('data-status');

                        const matchesQuery = !query || src.includes(query) || dest.includes(query) || note.includes(query);
                        const matchesCode = (codeFilter === 'all') || (code === codeFilter);
                        const matchesStatus = (statusFilter === 'all') || (status === statusFilter);

                        row.style.display = (matchesQuery && matchesCode && matchesStatus) ? '' : 'none';
                    });
                }

                if (redirectSearchInput) redirectSearchInput.addEventListener('input', filterRedirectRules);
                if (redirectFilterCode) redirectFilterCode.addEventListener('change', filterRedirectRules);
                if (redirectFilterStatus) redirectFilterStatus.addEventListener('change', filterRedirectRules);
                ...
                if (search404Input) {
                    search404Input.addEventListener('input', function() {
                        ...
                        rows.forEach(row => {
                            const uri = (row.querySelector('.gmb-404-uri-text')?.innerText || '').toLowerCase();
                ...
                if (bulkApplyBtn) {
                    bulkApplyBtn.addEventListener('click', function() {
                        const bulkAction = document.getElementById('gmb-bulk-redirect-action').value;
                        if (!bulkAction) {
                            alert('Please select a bulk action.');
                            return;
                        }

                        const selectedIds = Array.from(document.querySelectorAll('.gmb-rule-checkbox:checked')).map(cb => cb.value);
                        if (selectedIds.length === 0) {
                            alert('Please select at least one redirection rule.');
                            return;
                        }

                        if (bulkAction === 'delete' && !confirm('Are you sure you want to delete ' + selectedIds.length + ' redirection rule(s)?')) {
                            return;
                        }
                ...
                const bulkImportSubmitBtn = document.getElementById('gmb-bulk-import-submit-btn');
                if (bulkImportSubmitBtn) {
                    bulkImportSubmitBtn.addEventListener('click', function() {
                        ...
                        .then(data => {
                            ...
                            if (data.success) {
                                alert('Successfully imported ' + data.data + ' redirection rule(s)!');
                                window.location.reload();
                            } else {
                ...
                const fallbackSelect = document.getElementById('gmb_ranker_fallback_behavior');
                if (fallbackSelect) {
                ...
                const toggleRuleStatusBtns = document.querySelectorAll('.gmb-toggle-rule-status-btn');
                ...
                if (resetLinksBtn) {
                    resetLinksBtn.addEventListener('click', function(e) {
                        e.preventDefault();
                        if (confirm('Are you sure you want to reset all links options to default?')) {
                            const stripCheck = document.getElementById('gmb_strip_category_base');
                            if (stripCheck) stripCheck.checked = false;

                            const redirectCheck = document.getElementById('gmb_redirect_attachments');
                            if (redirectCheck) redirectCheck.checked = true;

                            const orphanInput = document.getElementById('gmb_redirect_orphan_attachments');
                            if (orphanInput) orphanInput.value = '';

                            const nofollowCheck = document.getElementById('gmb_nofollow_external_links');
                            if (nofollowCheck) nofollowCheck.checked = true;

                            const nofollowImgCheck = document.getElementById('gmb_nofollow_image_links');
                            if (nofollowImgCheck) nofollowImgCheck.checked = false;

                            const targetCheck = document.getElementById('gmb_new_window_external_links');
                            if (targetCheck) targetCheck.checked = true;

                            const affiliateInput = document.getElementById('gmb_affiliate_link_prefixes');
                ...
                if (resetLocalBtn) {
                    resetLocalBtn.addEventListener('click', function(e) {
                        e.preventDefault();
                        if (confirm('Are you sure you want to reset all Local SEO options to default?')) {
                ...
                const resetMetadataBtn = document.getElementById('gmb-reset-metadata-options');
                if (resetMetadataBtn) {
                    resetMetadataBtn.addEventListener('click', function(e) {
                        e.preventDefault();
                        if (confirm('Are you sure you want to reset all Metadata options to default?')) {
                            const postTitle = document.getElementById('gmb_metadata_post_title_template');
                            if (postTitle) postTitle.value = '%title% %sep% %sitename%';
                            
                            const postDesc = document.getElementById('gmb_metadata_post_desc_template');
                            if (postDesc) postDesc.value = '%excerpt%';
                            
                            const pageTitle = document.getElementById('gmb_metadata_page_title_template');
                            if (pageTitle) pageTitle.value = '%title% %sep% %sitename%';
                            
                            const pageDesc = document.getElementById('gmb_metadata_page_desc_template');
                ...
                (function() {
                    ...
                    document.addEventListener('click', function(e) {
                        ...
                        if (typeof wp === 'undefined' || !wp.media) {
                            alert('WordPress Media Library is unavailable. Please refresh the page.');
                            return;
                        }
                        ...
                        gmbMediaFrames[targetId].on('select', function() {
                            var attachment = gmbMediaFrames[targetId].state().get('selection').first().toJSON();
                            if (inputField) {
                ...
                if (resetImageBtn) {
                    resetImageBtn.addEventListener('click', function(e) {
                        e.preventDefault();
                        if (confirm('Are you sure you want to reset all Image SEO options to default?')) {
                ...
                const resetSecurityBtn = document.getElementById('gmb-reset-security-options');
                if (resetSecurityBtn) {
                    resetSecurityBtn.addEventListener('click', function(e) {
                        e.preventDefault();
                        if (confirm('Are you sure you want to reset all Security options to default?')) {
                            const xmlrpc = document.querySelector('input[name="gmb_seo_disable_xmlrpc"]');
                            if (xmlrpc) xmlrpc.checked = false;
                            
                            const wpVer = document.querySelector('input[name="gmb_seo_hide_wp_version"]');
                            if (wpVer) wpVer.checked = false;
                            
                            const rest = document.querySelector('input[name="gmb_seo_restrict_rest_api"]');
                            if (rest) rest.checked = false;
                            
                            const headers = document.querySelector('input[name="gmb_seo_enable_security_headers"]');
                ...
                function initSecurityHandlers() {
                    ...
                    if (secBtn) {
                        secBtn.addEventListener('click', function(e) {
                            e.preventDefault();
                            if (!confirm('Apply all recommended enterprise security hardening options now?')) return;
                            ...
                            .then(data => {
                                if (data.success) {
                                    alert(data.data.message || 'Security hardening applied successfully!');
                                    window.location.reload();
                                } else {
                            ...
                            btn.addEventListener('click', function(e) {
                                ...
                                rows.forEach(row => {
                                    if (filter === 'all') {
                                        ...
                                        row.style.display = (row.getAttribute('data-status') === 'issue') ? 'flex' : 'none';
                                    } else if (filter === 'passed') {
                                        row.style.display = (row.getAttribute('data-status') === 'passed') ? 'flex' : 'none';
                                    }
                                });
                            });
                        });
