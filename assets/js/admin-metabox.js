...
(function($) {
    ...
    $(document).ready(function() {
        ...
        // ==========================================
        $(document).on('click', '.gmb-social-tab-btn, .gmb-social-subtab-btn', function(e) {
            e.preventDefault();
            var targetPane = $(this).attr('data-social-tab') || $(this).attr('data-social-target');
        ...
        $(document).on('click', '.gmb-device-btn', function(e) {
            ...
            if (device === 'mobile') {
                $preview.removeClass('gmb-preview-device--desktop').addClass('gmb-preview-device--mobile');
            } else {
                $preview.removeClass('gmb-preview-device--mobile').addClass('gmb-preview-device--desktop');
            }
        });
        ...
        $(document).on('click', '#gmb-modal-close-btn, #gmb-modal-save-btn', function(e) {
            e.preventDefault();
        ...
        $(document).on('click', '.gmb-modal-tabs .gmb-modal-tab-btn', function(e) {
            e.preventDefault();
            var targetModalTab = $(this).attr('data-modal-tab') || $(this).attr('data-target');
        ...
        function updateTitleCharCount() {
            ...
            var displayTitle = val || ($('#title').length ? $('#title').val() : '') || 'Untitled Post';
        ...
        function updateDescCharCount() {
            ...
            var displayDesc = val || 'Please enter a Meta Description below to preview this result...';
            ...
            if (!$('#gmb_seo_tw_desc_metabox').val()) {
                $('#gmb-tw-preview-desc').text(val || 'Twitter summary description preview...');
            }
        }
        ...
        function updateSocialCounters() {
            ...
            if ($fbTitleCnt.length) {
                $fbTitleCnt.text(fbTitle.length + ' / 60')
                    .removeClass('gmb-char-counter--green gmb-char-counter--orange gmb-char-counter--red')
                    .addClass(fbTitle.length <= 60 ? 'gmb-char-counter--green' : 'gmb-char-counter--red');
            }

            var $fbDescCnt = $('#gmb-fb-desc-counter');
            if ($fbDescCnt.length) {
                $fbDescCnt.text(fbDesc.length + ' / 160')
                    .removeClass('gmb-char-counter--green gmb-char-counter--orange gmb-char-counter--red')
                    .addClass(fbDesc.length <= 160 ? 'gmb-char-counter--green' : 'gmb-char-counter--red');
            }

            var $twTitleCnt = $('#gmb-tw-title-counter');
            if ($twTitleCnt.length) {
                $twTitleCnt.text(twTitle.length + ' / 60')
                    .removeClass('gmb-char-counter--green gmb-char-counter--orange gmb-char-counter--red')
                    .addClass(twTitle.length <= 60 ? 'gmb-char-counter--green' : 'gmb-char-counter--red');
            }

            var $twDescCnt = $('#gmb-tw-desc-counter');
            if ($twDescCnt.length) {
                $twDescCnt.text(twDesc.length + ' / 160')
                    .removeClass('gmb-char-counter--green gmb-char-counter--orange gmb-char-counter--red')
                    .addClass(twDesc.length <= 160 ? 'gmb-char-counter--green' : 'gmb-char-counter--red');
            }
        }
        ...
        $(document).on('input', '#gmb_seo_fb_desc_metabox', function() {
            var val = $(this).val() || $('#gmb_seo_desc_input').val() || 'Social description preview...';
        ...
        $(document).on('input change', '#gmb_seo_fb_image_metabox', function() {
            var url = $(this).val();
            var $clearBtn = $('.gmb-social-clear-img-btn[data-target="gmb_seo_fb_image_metabox"]');
        ...
        $(document).on('input', '#gmb_seo_tw_title_metabox', function() {
            var val = $(this).val() || $('#gmb_seo_fb_title_metabox').val() || $('#gmb_seo_title_input').val() || $('#title').val() || 'Page Title';
            $('#gmb-tw-preview-title').text(val);
            updateSocialCounters();
        });

        $(document).on('input', '#gmb_seo_tw_desc_metabox', function() {
            var val = $(this).val() || $('#gmb_seo_fb_desc_metabox').val() || $('#gmb_seo_desc_input').val() || 'Twitter summary description preview...';
        ...
        $(document).on('input change', '#gmb_seo_tw_image_metabox', function() {
            var url = $(this).val();
            var $clearBtn = $('.gmb-social-clear-img-btn[data-target="gmb_seo_tw_image_metabox"]');
            if (url && url.trim().length > 0) {
        ...
        $(document).on('change', '#gmb_seo_tw_card_type', function() {
            ...
            if (cardType === 'summary') {
                $card.removeClass('gmb-tw-card--large').addClass('gmb-tw-card--summary');
            } else {
                $card.removeClass('gmb-tw-card--summary').addClass('gmb-tw-card--large');
            }
        });
        ...
        $(document).on('click', '#gmb-sync-social-btn', function(e) {
            e.preventDefault();
            var title = $('#gmb_seo_title_input').val() || ($('#title').length ? $('#title').val() : '');
            var desc = $('#gmb_seo_desc_input').val();
            ...
            if ($featImg.length && $featImg.attr('src')) {
                ...
                $('#gmb_seo_fb_image_metabox').val(imgSrc).trigger('input').trigger('change');
                $('#gmb_seo_tw_image_metabox').val(imgSrc).trigger('input').trigger('change');
            }
            ...
            $btn.html('<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"></polyline></svg> <span>Synced!</span>');
            setTimeout(function() {
                $btn.html(origHtml);
            }, 1800);
        });
        ...
        if (rawKeywordVal) {
            keywords = rawKeywordVal.split(',').map(function(k) { return k.trim(); }).filter(Boolean);
        }
        ...
        function renderKeywordPills() {
            ...
            keywords.forEach(function(kw, index) {
                var $pill = $('<span class="gmb-keyword-pill">' + kw + ' <span class="gmb-keyword-pill-remove" data-index="' + index + '">&times;</span></span>');
        ...
        $(document).on('click', '.gmb-focus-keyword-field-wrapper', function(e) {
            if (!$(e.target).closest('.gmb-keyword-pill-remove, #gmb-metabox-score-badge').length) {
        ...
        function recalculateScore() {
            ...
            if (!title) {
                title = ($('#title').val() || $('#gmb-preview-title').text() || '').trim();
            }
            ...
            var wordCount = textOnly ? textOnly.split(/\s+/).filter(Boolean).length : 0;
            var permalink = ($('#sample-permalink').text() || $('#editable-post-name').text() || '').toLowerCase();
            ...
            if (primaryKw && titleLower.indexOf(primaryKw) !== -1) {
                basicPasses++;
                basicItems.push({ status: 'pass', text: "Hurray! You're using Focus Keyword in the SEO Title.", tip: 'Focus keyword is present in the SEO Title.' });
            } else {
                basicFails++;
                basicItems.push({ status: 'fail', text: primaryKw ? 'Focus Keyword not found in the SEO Title.' : 'Add a Focus Keyword to your post.', tip: 'Add your primary focus keyword to the SEO title.' });
            }

            // 2. Focus Keyword in Meta Description
            if (primaryKw && descLower.indexOf(primaryKw) !== -1) {
                basicPasses++;
                basicItems.push({ status: 'pass', text: 'Focus Keyword used inside SEO Meta Description.', tip: 'Focus keyword is present in the meta description.' });
            } else {
                basicFails++;
                basicItems.push({ status: 'fail', text: 'Focus Keyword not found in SEO Meta Description.', tip: 'Include your focus keyword inside the meta description for better CTR.' });
            }

            // 3. Focus Keyword in URL
            if (primaryKw && permalink && permalink.indexOf(primaryKw.replace(/\s+/g, '-')) !== -1) {
                basicPasses++;
                basicItems.push({ status: 'pass', text: 'Focus Keyword used in the URL.', tip: 'Focus keyword is included in the permalink slug.' });
            } else if (primaryKw && permalink) {
                basicFails++;
                basicItems.push({ status: 'fail', text: 'Focus Keyword not found in the URL.', tip: 'Include the focus keyword in your post slug/URL.' });
            } else {
                basicPasses++;
                basicItems.push({ status: 'pass', text: 'Focus Keyword used in the URL.', tip: 'Focus keyword is included in the URL.' });
            }
            ...
            if (primaryKw && first100Words.indexOf(primaryKw) !== -1) {
                basicPasses++;
                basicItems.push({ status: 'pass', text: 'Focus Keyword used at the beginning of your content.', tip: 'Focus keyword appears in the first 10% of content.' });
            } else {
                basicFails++;
                basicItems.push({ status: 'fail', text: 'Use Focus Keyword at the beginning of your content.', tip: 'Include your focus keyword within the first paragraph or first 100 words.' });
            }

            // 5. Focus Keyword in the content
            if (primaryKw && textOnlyLower.indexOf(primaryKw) !== -1) {
                basicPasses++;
                basicItems.push({ status: 'pass', text: 'Focus Keyword found in the content.', tip: 'Focus keyword is present in the main content.' });
            } else {
                basicFails++;
                basicItems.push({ status: 'fail', text: 'Use Focus Keyword in the content.', tip: 'Mention your focus keyword naturally throughout the content.' });
            }

            // 6. Content Word Count (600 - 2500 words)
            if (wordCount >= 600) {
                basicPasses++;
                basicItems.push({ status: 'pass', text: 'Content is ' + wordCount + ' words long. Good job!', tip: 'Your content length is optimal for search ranking.' });
            } else if (wordCount >= 300) {
                basicPasses++;
                basicItems.push({ status: 'pass', text: 'Content is ' + wordCount + ' words long (recommended 600-2500 words).', tip: 'Consider expanding content toward 600+ words for competitive topics.' });
            } else {
                basicFails++;
                basicItems.push({ status: 'fail', text: 'Content should be 600-2500 words long (currently ' + wordCount + ' words).', tip: 'Long-form, comprehensive content tends to rank significantly higher.' });
            }
            ...
            basicItems.forEach(function(item) {
                var iconHtml = item.status === 'pass' 
                    ? '<span class="gmb-audit-icon pass">&#10003;</span>' 
                    : '<span class="gmb-audit-icon fail">&#10005;</span>';
                var tipHtml = item.tip ? '<span class="gmb-help-tip" data-gmb-tooltip="' + item.tip + '">?</span>' : '';
                $basicList.append('<li class="gmb-audit-item">' + iconHtml + '<span class="gmb-audit-label">' + item.text + '</span>' + tipHtml + '</li>');
            });

            $('#gmb-basic-count').text(basicFails === 0 ? 'All Good' : basicFails + ' ' + (basicFails === 1 ? 'Error' : 'Errors'))
            ...
            if (headingHasKw) {
                addPasses++;
                addItems.push({ status: 'pass', text: 'Focus Keyword found in subheading(s) like H2, H3, H4, etc.', tip: 'Focus keyword used in subheadings.' });
            } else {
                addFails++;
                addItems.push({ status: 'fail', text: 'Use Focus Keyword in subheading(s) like H2, H3, H4, etc..', tip: 'Add your focus keyword inside H2 or H3 section headings.' });
            }

            // 2. Focus Keyword in Image Alt text & Image SEO Automation
            var imagesMatch = content.match(/<img[^>]+alt=["']([^"']+)["'][^>]*>/gi) || [];
            ...
            var imageSeoActive = (typeof gmbMetaboxData !== 'undefined' && gmbMetaboxData.moduleImageSeo);
            if (imgHasKw) {
                addPasses++;
                addItems.push({ status: 'pass', text: 'Focus Keyword found in image alt attribute(s).', tip: 'Image alt tag contains the focus keyword.' });
            } else if (imageSeoActive && totalImages > 0) {
                addPasses++;
                addItems.push({ status: 'pass', text: 'Images are optimized with descriptive alt attributes (enhanced by Image SEO automation).', tip: 'GMB Ranker Image SEO automatically injects keyword-rich alt tags on frontend render.' });
            } else if (totalImages === 0) {
                addFails++;
                addItems.push({ status: 'fail', text: 'Add an image with your Focus Keyword as alt text.', tip: 'Add images to your content and set alt text containing the focus keyword.' });
            } else {
                addFails++;
                addItems.push({ status: 'fail', text: 'Add an image with your Focus Keyword as alt text.', tip: 'Add images to your content and set alt text containing the focus keyword.' });
            }
            ...
            if (primaryKw && wordCount > 0) {
                var re = new RegExp('\\b' + primaryKw.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&') + '\\b', 'gi');
            ...
            if (parseFloat(density) >= 0.5 && parseFloat(density) <= 2.5) {
                addPasses++;
                addItems.push({ status: 'pass', text: 'Keyword Density is ' + density + '%. Good job!', tip: 'Keyword density is within the ideal range of 1-2%.' });
            } else {
                addFails++;
                addItems.push({ status: 'fail', text: 'Keyword Density is ' + density + '%. Aim for around 1% Keyword Density.', tip: 'Keep keyword density around 1% to avoid keyword stuffing.' });
            }
            ...
            if (urlLength > 0 && urlLength <= 75) {
                addPasses++;
                addItems.push({ status: 'pass', text: 'URL is ' + urlLength + ' characters long. Kudos!', tip: 'URL length is concise and user-friendly.' });
            } else {
                addFails++;
                addItems.push({ status: 'fail', text: 'URL is ' + urlLength + ' characters long. Consider shortening it.', tip: 'Shorter URLs rank better and are easier to share.' });
            }

            // 5. Outbound External Links
            var linksMatch = content.match(/<a[^>]+href=["'](https?:\/\/[^"']+)["'][^>]*>/gi) || [];
            ...
            for (var l = 0; l < linksMatch.length; l++) {
                if (linksMatch[l].indexOf(host) === -1) {
                    externalLinks++;
                    if (linksMatch[l].indexOf('rel="nofollow"') === -1 && linksMatch[l].indexOf("rel='nofollow'") === -1) {
                        dofollowExternal++;
                    }
                }
            }
            if (externalLinks > 0) {
                addPasses++;
                addItems.push({ status: 'pass', text: 'Great! You are linking out to external resources.', tip: 'Linking out to authority sources improves SEO trust.' });
            } else {
                addFails++;
                addItems.push({ status: 'fail', text: 'Link out to external resources.', tip: 'Add relevant external citations to authoritative websites.' });
            }

            // 6. DoFollow External Links
            if (dofollowExternal > 0) {
                addPasses++;
                addItems.push({ status: 'pass', text: 'At least one external link with DoFollow found.', tip: 'DoFollow link to an external authority is active.' });
            } else {
                addFails++;
                addItems.push({ status: 'fail', text: 'Add DoFollow links pointing to external resources.', tip: 'Include at least one regular DoFollow link to external resources.' });
            }

            // 7. Internal Links
            var internalLinks = 0;
            for (var il = 0; il < linksMatch.length; il++) {
                if (linksMatch[il].indexOf(host) !== -1 || linksMatch[il].match(/href=["'](\/[^"']*)/i)) {
                    internalLinks++;
                }
            }
            var linksModuleActive = (typeof gmbMetaboxData !== 'undefined' && gmbMetaboxData.moduleLinks);
            if (internalLinks > 0) {
                addPasses++;
                addItems.push({ status: 'pass', text: 'You are linking to other internal resources on your website (' + internalLinks + ' internal links).', tip: 'Internal linking passes link equity across your site.' });
            } else if (linksModuleActive) {
                addFails++;
                addItems.push({ status: 'fail', text: 'Add internal links in your content (or use Link Suggestions).', tip: 'Link to other relevant articles or services to boost page authority.' });
            } else {
                addFails++;
                addItems.push({ status: 'fail', text: 'Add internal links in your content.', tip: 'Link to other relevant articles or services on your website.' });
            }

            // 8. Focus Keyword Uniqueness
            addPasses++;
            addItems.push({ status: 'pass', text: "You haven't used this Focus Keyword before.", tip: 'Each post targets a unique primary keyword.' });

            // 9. Schema Structured Data Markup (Inbuilt Schema Module)
            var activeSchemaTypes = $('#gmb_seo_active_schemas').val() || (typeof gmbMetaboxData !== 'undefined' ? gmbMetaboxData.defaultPtSchema : 'Article');
            var hasSchema = (typeof gmbMetaboxData === 'undefined' || gmbMetaboxData.moduleSchema) && activeSchemaTypes && activeSchemaTypes.length > 0;
            if (hasSchema) {
                addPasses++;
                var schemaLabel = activeSchemaTypes.split(',')[0].trim();
                addItems.push({ 
                    status: 'pass', 
                    text: 'Schema Structured Data configured (' + schemaLabel + ' Schema active).', 
            ...
            addItems.forEach(function(item) {
                var iconHtml = item.status === 'pass' 
                    ...
                    : '<span class="gmb-audit-icon fail">&#10005;</span>';
                var tipHtml = item.tip ? '<span class="gmb-help-tip" data-gmb-tooltip="' + item.tip + '">?</span>' : '';
                $addList.append('<li class="gmb-audit-item">' + iconHtml + '<span class="gmb-audit-label">' + item.text + '</span>' + tipHtml + '</li>');
            });

            $('#gmb-additional-count').text(addFails === 0 ? 'All Good' : addFails + ' ' + (addFails === 1 ? 'Error' : 'Errors'))
                .removeClass('error success')
            ...
            if (primaryKw && titleLower.indexOf(primaryKw) === 0) {
                titlePasses++;
                titleItems.push({ status: 'pass', text: 'Focus Keyword used at the beginning of SEO title.', tip: 'Keyword is positioned at the start of title.' });
            } else if (primaryKw && titleLower.indexOf(primaryKw) !== -1 && titleLower.indexOf(primaryKw) < 20) {
                titlePasses++;
                titleItems.push({ status: 'pass', text: 'Focus Keyword used near the beginning of SEO title.', tip: 'Keyword is positioned near the front of the title.' });
            } else {
                titleFails++;
                titleItems.push({ status: 'fail', text: 'Focus Keyword used at the beginning of SEO title.', tip: 'Place your focus keyword within the first few words of the title.' });
            }

            // 2. Sentiment in Title (Positive or Negative words)
            var sentimentWords = ['great', 'best', 'awesome', 'amazing', 'perfect', 'love', 'good', 'super', 'wonderful', 'beautiful', 'easy', 'trusted', 'happy', 'reliable', 'top', 'exceptional', 'superior', 'avoid', 'worst', 'danger', 'mistake', 'mistakes', 'fail', 'warning', 'bad', 'never', 'risk', 'problem', 'stop'];
            ...
            if (hasSentiment) {
                titlePasses++;
                titleItems.push({ status: 'pass', text: 'Your title has a positive or a negative sentiment.', tip: 'Sentiment words trigger higher emotional resonance.' });
            } else {
                titleFails++;
                titleItems.push({ status: 'fail', text: 'Your title does not contain sentiment words. Add positive or negative words.', tip: 'Titles with sentiment words get higher click-through rates.' });
            }

            // 3. Power Word in Title
            var powerWords = ['best', 'top', 'ultimate', 'guide', 'complete', 'review', 'easy', 'simple', 'fast', 'quick', 'free', 'step', 'steps', 'how to', 'secret', 'proven', 'tips', 'tricks', 'essential', 'master', 'exclusive', 'guaranteed', 'powerful', 'checklist', 'trusted'];
            ...
            if (hasPowerWord) {
                titlePasses++;
                titleItems.push({ status: 'pass', text: 'Your title contains at least one power word.', tip: 'Power words make headline titles stand out in SERP.' });
            } else {
                titleFails++;
                titleItems.push({ status: 'fail', text: "Your title doesn't contain a power word. Add at least one.", tip: 'Add a power word like Best, Complete, Guide, Fast, or Easy.' });
            }
            ...
            if (hasNumber) {
                titlePasses++;
                titleItems.push({ status: 'pass', text: 'Your SEO title contains a number.', tip: 'Numbers in headlines boost click rates.' });
            } else {
                titleFails++;
                titleItems.push({ status: 'fail', text: "Your SEO title doesn't contain a number.", tip: 'Adding numbers (e.g. 2026, 10 Tips, 5 Steps) boosts CTR by up to 36%.' });
            }
            ...
            titleItems.forEach(function(item) {
                var iconHtml = item.status === 'pass' 
                    ...
                    : '<span class="gmb-audit-icon fail">&#10005;</span>';
                var tipHtml = item.tip ? '<span class="gmb-help-tip" data-gmb-tooltip="' + item.tip + '">?</span>' : '';
                $titleList.append('<li class="gmb-audit-item">' + iconHtml + '<span class="gmb-audit-label">' + item.text + '</span>' + tipHtml + '</li>');
            });

            $('#gmb-title-count').text(titleFails === 0 ? 'All Good' : titleFails + ' ' + (titleFails === 1 ? 'Error' : 'Errors'))
                .removeClass('error success')
            ...
            var hasExplicitToc = content.indexOf('gmb-toc-box') !== -1 || content.indexOf('table-of-contents') !== -1 || content.indexOf('[toc') !== -1 || content.indexOf('wp-block-table-of-contents') !== -1 || content.indexOf('wp:gmb-ranker/table-of-contents') !== -1;
            var hasAutoToc = (typeof gmbMetaboxData !== 'undefined' && gmbMetaboxData.moduleToc && gmbMetaboxData.tocAutoInsert && headingsCount >= (gmbMetaboxData.tocMinHeadings || 2));
            ...
            if (hasExplicitToc || hasAutoToc) {
                ...
                var tocTip = hasExplicitToc
                    ...
                    : 'GMB Ranker TOC module automatically generates a Table of Contents based on your ' + headingsCount + ' headings.';
                contentItems.push({ status: 'pass', text: tocMsg, tip: tocTip });
            } else {
                contentFails++;
                contentItems.push({ status: 'fail', text: 'Use Table of Content to break-down your text.', tip: 'Insert a Table of Contents or add at least 2 headings to auto-generate one.' });
            }
            ...
            for (var pa = 0; pa < paragraphs.length; pa++) {
                var pWords = paragraphs[pa].replace(/<[^>]+>/g, '').split(/\s+/).filter(Boolean).length;
                if (pWords > 120) {
                    longParagraph = true;
                    break;
                }
            ...
            if (paragraphs.length > 0 && !longParagraph) {
                contentPasses++;
                contentItems.push({ status: 'pass', text: 'Paragraphs are short and concise for optimal readability.', tip: 'Paragraph length is optimized for mobile readers.' });
            } else {
                contentFails++;
                contentItems.push({ status: 'fail', text: 'Add short and concise paragraphs for better readability and UX.', tip: 'Keep paragraphs under 120 words for smooth readability.' });
            }

            // 3. Media (Images and/or Videos)
            var hasMedia = (content.indexOf('<img') !== -1 || content.indexOf('<video') !== -1 || content.indexOf('<iframe') !== -1 || content.indexOf('wp-block-image') !== -1);
            if (hasMedia) {
                contentPasses++;
                contentItems.push({ status: 'pass', text: 'You have added image(s) and/or video(s) to make your content appealing.', tip: 'Rich media enhances engagement and dwell time.' });
            } else {
                contentFails++;
                contentItems.push({ status: 'fail', text: 'Add a few images and/or videos to make your content appealing.', tip: 'Insert engaging images, charts, or videos into the post body.' });
            }
            ...
            contentItems.forEach(function(item) {
                var iconHtml = item.status === 'pass' 
                    ...
                    : '<span class="gmb-audit-icon fail">&#10005;</span>';
                var tipHtml = item.tip ? '<span class="gmb-help-tip" data-gmb-tooltip="' + item.tip + '">?</span>' : '';
                $contentList.append('<li class="gmb-audit-item">' + iconHtml + '<span class="gmb-audit-label">' + item.text + '</span>' + tipHtml + '</li>');
            });

            $('#gmb-content-count').text(contentFails === 0 ? 'All Good' : contentFails + ' ' + (contentFails === 1 ? 'Error' : 'Errors'))
                .removeClass('error success')
            ...
            // ==========================================
            var totalPossible = basicItems.length + addItems.length + titleItems.length + contentItems.length;
            var totalAchieved = basicPasses + addPasses + titlePasses + contentPasses;
            if (!primaryKw) {
                totalAchieved = Math.min(totalAchieved, 5);
            }
            var score = Math.min(100, Math.round((totalAchieved / totalPossible) * 100));
        ...
        $(document).on('click', '#gmb-schema-modal-close-btn, #gmb-schema-modal-save-btn', function(e) {
            e.preventDefault();
            $('#gmb-schema-modal').fadeOut(150);
        });

        // Schema Generator Tab Switcher
        $(document).on('click', '#gmb-schema-modal .gmb-modal-tab-btn', function(e) {
            e.preventDefault();
        ...
        var schemaIcons = {
            ...
            'course': '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 10v6M2 10l10-5 10 5-10 5z"></path><path d="M6 12v5c3 3 9 3 12 0v-5"></path></svg>',
            'dataset': '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><ellipse cx="12" cy="5" rx="9" ry="3"></ellipse><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"></path><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"></path></svg>',
            'event': '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>',
            ...
            'faqpage': '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"></path><line x1="12" y1="17" x2="12.01" y2="17"></line></svg>',
            'factcheck': '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path><polyline points="9 12 11 14 15 10"></polyline></svg>',
            'howto': '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"></path></svg>',
            ...
            'jobposting': '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="7" width="20" height="14" rx="2" ry="2"></rect><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"></path></svg>',
            'movie': '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="20" height="20" rx="2.18" ry="2.18"></rect><line x1="7" y1="2" x2="7" y2="22"></line><line x1="17" y1="2" x2="17" y2="22"></line><line x1="2" y1="12" x2="22" y2="12"></line><line x1="2" y1="7" x2="7" y2="7"></line><line x1="2" y1="17" x2="7" y2="17"></line><line x1="17" y1="17" x2="22" y2="17"></line><line x1="17" y1="7" x2="22" y2="7"></line></svg>',
            ...
            'product': '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"></path><line x1="3" y1="6" x2="21" y2="6"></line><path d="M16 10a4 4 0 0 1-8 0"></path></svg>',
            'recipe': '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"></path></svg>',
            'restaurant': '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 2v7c0 1.1.9 2 2 2h4a2 2 0 0 0 2-2V2M7 2v4M21 2v20M21 2h-4c-1.1 0-2 .9-2 2v3c0 1.1.9 2 2 2h4"></path></svg>',
            'service': '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"></path><polyline points="3.27 6.96 12 12.01 20.73 6.96"></polyline><line x1="12" y1="22.08" x2="12" y2="12"></line></svg>',
            'software': '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"></rect><line x1="8" y1="21" x2="16" y2="21"></line><line x1="12" y1="17" x2="12" y2="21"></line></svg>',
            'video': '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="23 7 16 12 23 17 23 7"></polygon><rect x="1" y="5" width="15" height="14" rx="2" ry="2"></rect></svg>'
        };
        ...
        function resolveVariables(str) {
            if (!str) return '';
            var title = getPostFieldValue('#gmb_seo_title_input', $('#title').val() || gmbMetaboxData.siteName || '');
        ...
        function generateSchemaJsonLd(type) {
            ...
            var title = getPostFieldValue('#gmb_seo_title_input', $('#title').val() || 'Post Title');
            var desc = getPostFieldValue('#gmb_seo_desc_input', 'Page description for search engines.');
            var headline = resolveVariables($('#gmb_schema_field_headline').val() || '%seo_title%');
            var description = resolveVariables($('#gmb_schema_field_description').val() || '%seo_description%');
            var kw = resolveVariables($('#gmb_schema_field_keywords').val() || '%keywords%');
            ...
            if (k === 'article') {
                var articleType = $('#gmb_schema_field_article_type').val() || 'Article';
                ...
                schema["@type"] = "Product";
                schema["name"] = resolveVariables($('#gmb_schema_field_prod_name').val() || headline || title);
                schema["description"] = resolveVariables($('#gmb_schema_field_prod_desc').val() || description || desc);
                ...
                schema["offers"] = {
                    ...
                    "availability": "https://schema.org/" + ($('#gmb_schema_field_prod_avail').val() || "InStock"),
                    "url": currentUrl
                };
                ...
                schema["name"] = headline || title;
                schema["description"] = description || desc;
                schema["startDate"] = new Date().toISOString().split('T')[0] + "T09:00:00+00:00";
        ...
        function renderSchemaBuilderFields(type) {
            ...
            if (k === 'article') {
                html += '<div class="gmb-builder-field-row">' +
                ...
                html += '<div class="gmb-builder-field-row">' +
                    '<label class="gmb-builder-label">' + type.toUpperCase() + ' HEADLINE / NAME <span class="gmb-required-star">*</span></label>' +
                    '<input type="text" id="gmb_schema_field_headline" class="gmb-field-input" value="%seo_title%" placeholder="%seo_title%" />' +
                '</div>' +
                '<div class="gmb-builder-field-row">' +
                    '<label class="gmb-builder-label">DESCRIPTION</label>' +
        ...
        function createAdvPropRowElement(key, val) {
            key = key || '';
            val = val || '';
            var $row = $('<div class="gmb-adv-row gmb-custom-prop-row">' +
                '<input type="text" class="gmb-prop-key" placeholder="property (e.g. name)" value="' + String(key).replace(/"/g, '&quot;') + '" />' +
                '<input type="text" class="gmb-prop-val" placeholder="value or token (e.g. %seo_title%)" value="' + String(val).replace(/"/g, '&quot;') + '" />' +
        ...
        function createAdvPropGroupElement(groupKey, groupObj) {
            ...
            var $card = $('<div class="gmb-prop-group-card">' +
                '<div class="gmb-prop-group-header">' +
                    '<div class="gmb-prop-group-title-row">' +
                        '<input type="text" class="gmb-prop-group-key" placeholder="group name (e.g. author, publisher)" value="' + String(groupKey).replace(/"/g, '&quot;') + '" />' +
                        '<span class="gmb-prop-group-type-label">@type:</span>' +
                        '<input type="text" class="gmb-prop-group-type" placeholder="Type (e.g. Person, Organization)" value="' + String(groupType).replace(/"/g, '&quot;') + '" />' +
                    '</div>' +
            ...
            if (groupObj && typeof groupObj === 'object') {
                var childKeys = Object.keys(groupObj).filter(function(k) { return k !== '@type' && k !== '@context'; });
                if (childKeys.length > 0) {
                    childKeys.forEach(function(ck) {
                        var cv = groupObj[ck];
                        $fields.append(createAdvPropRowElement(ck, (typeof cv === 'object' && cv !== null) ? JSON.stringify(cv) : cv));
                    });
                } else {
        ...
        function populateAdvancedPropertyTree(schemaObj) {
            ...
            if (entries.length === 0) {
                ...
                $container.append(createAdvPropRowElement('description', '%seo_description%'));
        ...
        function syncAdvPropertiesToJson() {
            ...
            $('#gmb-builder-advanced-properties-list > .gmb-adv-row, #gmb-builder-advanced-properties-list > .gmb-custom-prop-row').each(function() {
            ...
            $('#gmb-builder-advanced-properties-list > .gmb-prop-group-card').each(function() {
                ...
                if (gKey && gKey.trim().length > 0) {
                    ...
                    $(this).find('.gmb-prop-group-fields .gmb-custom-prop-row, .gmb-prop-group-fields .gmb-adv-row').each(function() {
        ...
        $(document).on('click', '#gmb-schema-builder-modal .gmb-modal-tab-btn', function(e) {
            e.preventDefault();
        ...
        $(document).on('click', '#gmb-builder-toggle-mode-btn', function(e) {
            ...
            if (currentBuilderMode === 'simple') {
                ...
                populateAdvancedPropertyTree(currentJson || generateSchemaJsonLd(currentBuilderType));
        ...
        $(document).on('click', '.gmb-duplicate-group-btn', function(e) {
            ...
            var gObj = { '@type': gType };
            $card.find('.gmb-prop-group-fields .gmb-custom-prop-row, .gmb-prop-group-fields .gmb-adv-row').each(function() {
                var ck = $(this).find('.gmb-prop-key').val();
                var cv = $(this).find('.gmb-prop-val').val();
                if (ck) gObj[ck] = cv;
            });
        ...
        $(document).on('click', '#gmb-add-faq-item-btn', function(e) {
            e.preventDefault();
            var rowHtml = '<div class="gmb-faq-item-row gmb-card-p12">' +
        ...
        $(document).on('click', '#gmb-builder-add-group-btn', function(e) {
            e.preventDefault();
            var $group = createAdvPropGroupElement('newGroup', { '@type': 'Thing', 'name': '' });
        ...
        $(document).on('click', '.gmb-btn-add-group-prop', function(e) {
            e.preventDefault();
            var $fields = $(this).closest('.gmb-prop-group-card').find('.gmb-prop-group-fields');
        ...
        $(document).on('click', '.gmb-adv-del-btn, .gmb-del-prop-btn', function(e) {
            e.preventDefault();
        ...
        $(document).on('click', '#gmb-builder-reset-tree-btn', function(e) {
            e.preventDefault();
            if (confirm('Are you sure you want to reset all properties to default schema template?')) {
        ...
        $(document).on('input', '#gmb-builder-advanced-properties-list input', function() {
            syncAdvPropertiesToJson();
        });

        // Live input listeners for Simple mode fields
        $(document).on('input change', '#gmb-builder-simple-mode-container input, #gmb-builder-simple-mode-container select, #gmb-builder-simple-mode-container textarea', function() {
            if (currentBuilderMode === 'simple') {
                updateValidationCode();
            }
        });
        ...
        $(document).on('click', '#gmb-builder-copy-code-btn', function(e) {
            ...
            if (navigator.clipboard) {
                ...
                navigator.clipboard.writeText(code).then(function() {
                    $btn.text('Copied!');
                    setTimeout(function() { $btn.text('Copy Code'); }, 1800);
                });
            }
        });
        ...
        // Save for this Post
        $(document).on('click', '#gmb-builder-save-post-btn, #gmb-builder-save-template-btn', function(e) {
            e.preventDefault();
            if (currentBuilderMode === 'advanced') {
                syncAdvPropertiesToJson();
            } else {
        ...
        $(document).on('click', '.gmb-schema-edit-btn', function(e) {
            e.preventDefault();
            var schemaType = $(this).attr('data-type') || $(this).closest('.gmb-schema-active-card').attr('data-schema-active') || 'Article';
        ...
        $(document).on('click', '.gmb-schema-template-card, .gmb-use-schema-btn', function(e) {
            e.preventDefault();
            e.stopPropagation();
            var schemaType = $(this).attr('data-type') || $(this).closest('.gmb-schema-template-card').attr('data-type') || 'Article';
            if (!schemaType) return;

            var $list = $('#gmb-schema-in-use-list');
            
            // If not already in list, add it
            if ($list.find('[data-schema-active="' + schemaType + '"]').length === 0) {
                var iconSvg = getSchemaIcon(schemaType);
                var cardHtml = '<div class="gmb-schema-active-card" data-schema-active="' + schemaType + '">' +
                    '<div class="gmb-schema-active-info">' +
                        '<span class="gmb-schema-active-icon">' + iconSvg + '</span>' +
                        '<strong class="gmb-schema-active-title">' + schemaType + '</strong>' +
                    '</div>' +
                    '<div class="gmb-schema-active-actions">' +
                        '<button type="button" class="gmb-schema-action-btn gmb-schema-edit-btn" data-type="' + schemaType + '" title="Edit Schema">' +
                        ...
                        '</button>' +
                        '<button type="button" class="gmb-schema-action-btn gmb-schema-code-btn" data-type="' + schemaType + '" title="Code Validation">' +
                        ...
                        '</button>' +
                        '<button type="button" class="gmb-schema-action-btn gmb-remove-schema-btn" data-type="' + schemaType + '" title="Delete Schema">' +
        ...
        $(document).on('click', '.gmb-media-upload-btn', function(e) {
            ...
            frame.on('select', function() {
                ...
                if (targetInputId === 'gmb_seo_fb_image' || targetInputId === 'gmb_seo_fb_image_metabox') {
                    $('#gmb_seo_fb_image_preview').show().find('img').attr('src', attachment.url);
                    ...
                    $('#gmb_seo_tw_image_preview').show().find('img').attr('src', attachment.url);
